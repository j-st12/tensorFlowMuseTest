...
class InputData:
    ...
    def add_dataaa(self, path_x, path_y, key='train'):
        self.x[key] = path_x
        self.y[key] = path_y
        print('data size x:', self.x[key].shape)
        print('data size y:', self.y[key].shape)

    def add_dataaa2(self, path_x, path_y, key='train', batch_size=None):
        # Load data from .npy files
        x_data = np.load(path_x)
        y_data = np.load(path_y)

        # Assuming your desired shape is (batch_size, 48, 84, 5)
        #desired_shape = (x_data.shape[0], 48, 84, 5)

        # Check and reshape data if necessary
        #if x_data.shape != desired_shape:
         #   x_data = x_data.reshape(desired_shape)

        #if y_data.shape != (batch_size, ...):  # Adjust the desired shape for y_data
         #   y_data = y_data.reshape(desired_shape)
